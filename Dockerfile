...
ARG EXO_VERSION=6.2.x-clustering-SNAPSHOT
...
COPY ./plf-enterprise-tomcat-standalone.zip /srv/downloads/eXo-Platform-${EXO_VERSION}.zip
...
RUN unzip -q /srv/downloads/eXo-Platform-${EXO_VERSION}.zip -d /srv/downloads/ && \
  ls /srv/downloads/ && \
